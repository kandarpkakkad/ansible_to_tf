...
class RelatedResourceScraper:
    ...
    def collect_sqs_queue(self, queue_arn: str) -> None:
        """Collect SQS queue and related resources"""
        ...
        try:
            ...
            if queue.get('KmsMasterKeyId'):
                self.collect_kms_key(queue['KmsMasterKeyId'])
            ...
            self.resource.depends_on.append(f"sqs:{queue_arn}")
            
        except Exception as e:
            self.scraper.logger.error(f"Error collecting related SQS queue {queue_arn}: {str(e)}")
    ...
    def collect_s3_bucket(self, bucket_name: str) -> None:
        """Collect S3 bucket and its configurations"""
        if bucket_name in self.scraped_resources:
            return
        
        try:
            s3 = self.scraper.get_client('s3')
            ...
            try:
                ...
                if 'ServerSideEncryptionConfiguration' in encryption:
                    ...
                    # Collect KMS key if used
                    for rule in encryption['ServerSideEncryptionConfiguration'].get('Rules', []):
            ...
            self.scraped_resources.add(bucket_name)
            
            self.resource.depends_on.append(f"s3:bucket:{bucket_name}")
            
        except ClientError as e:
            self.scraper.handle_aws_error('process_bucket', bucket_name, e)
        except Exception as e:
            self.scraper.logger.error(f"Error collecting related S3 bucket {bucket_name}: {str(e)}")
